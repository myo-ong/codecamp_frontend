...
const resolvers = {
  ...
  Mutation: {
    ...
    //   await Board.update({ number: 3 }, { isDeleted: true }); // 3번 게시글 삭제했다치자 (소프트삭제) => isDeleted가 초기값이 false이면? 삭제 안된거, true이면? 삭제 된거
    //   await Board.update({ number: 3 }, { deletedAt: new Date() }); // 3번 게시글 삭제했다치자 (소프트삭제) => deletedAt 초기값이 NULL이면? 삭제 안된거, new Date()가 들어가있으면? 삭제 된거
    // },
  },
};
